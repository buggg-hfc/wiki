...
对于 $j$，有 $s[j] \cdots s[j+z[j]-1] \ = \ s[1] \cdots s[z[j]]$

为了计算 $z[i]$，在枚举 $i$ 的过程中，我们需要维护一个 $R$ 最大的区间 $[L,R]$，其中 $\begin{cases}L=j& (j<i)\\R=j+z[j]-1& (j<i)\end{cases}$
...
1. 若 $i \leq R$，则根据定义有 $s[L] \cdots s[R]\ =\ s[1] \cdots s[R-L+1]$
    ...
    - 若 $z[k] \ge R-i+1$，说明从 $k$ 开始可以匹配到 $R-L+1$ 那么远，也就是说从 $i$ 开始可以匹配到 $R$ 那么远，此时从 $R+1$ 开始继续暴力向后匹配即可，即 $z[i]=R-i+1$ 并继续匹配
